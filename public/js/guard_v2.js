...
class GuardApp {
    ...
    static updateUIForNextPoint() {
        if (this.activeRoute && this.activeRoute.points && this.currentSequenceIndex < this.activeRoute.points.length) {
            ...
            const ptName = nextPoint ? nextPoint.name : "Punto Desconocido";
            const el = document.getElementById('next-checkpoint-name');
            if (el) el.textContent = ptName;
            ...
            const el = document.getElementById('next-checkpoint-name');
            if (el) el.textContent = "Ronda Completada";
            this.completeRound();
        }
    }
